...
本项目以"上市公司年度报告智能问答"为场景，构建一套接近企业级落地标准的 RAG（检索增强生成）系统。数据来自巨潮资讯网（证监会指定披露平台），包含
5 家公司 × 3 年 = 15 份年报 PDF，总量约 85 MB。
...
选型对比教学价值：两种方案召回结果对同一问题给出相同答案（`14,769,360.50万元`），但BGE 完全离线，text-embedding-v3 按量计费、无需本地
GPU。
...
**可选查询改写**（`--query-rewrite`）： 用 `qwen-turbo` 将模糊问题改写为检索友好形式，例如：
