...
A: 评估口径已统一（均为 span F1 + text.find ()，与 seqeval 差 < 0.01），差距是真实的。根本原因是任务特性：NER
需要精确定位每个字符的边界，BIO 序列标注天然把边界信息编码进标签，每个 token 的预测结果就是位置信息本身；而生成式方法输出自然语言
JSON，边界只能靠 text.find () 近似还原。此外当前 SFT 只跑了 1 epoch，3 epoch 预计还能提升 3~5 个点。
