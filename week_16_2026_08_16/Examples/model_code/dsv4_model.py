...
class Gate(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        if self.hash:
            self.tid2eid = nn.Parameter(torch.empty(args.vocab_size, args.n_activated_experts, dtype=torch.int32),
                                        requires_grad=False)
            self.bias = None
        else:
...
class MoE(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.experts = nn.ModuleList([Expert(args.dim, args.moe_inter_dim, dtype=expert_dtype,
                                             swiglu_limit=args.swiglu_limit) if self.experts_start_idx <= i < self.experts_end_idx else None
